...
SDL_PrivateSendMouseMotion(SDL_Window * window, SDL_MouseID mouseID, int relative, int x, int y)
{
    ...
    if (relative) {
        ...
        /*  xrel and yrel will continue to add/substract outside screen, 
            so clamp x and y to the screenWidth and screenHeight */
        int windowWidth = 0, windowHeight = 0;
        SDL_GetWindowSize(window, &windowWidth, &windowHeight);
        x = (mouse->last_x + xrel > 0 ? mouse->last_x + xrel : 0);
        x = (mouse->last_x + xrel < windowWidth ? x : windowWidth);
        y = (mouse->last_y + yrel > 0 ? mouse->last_y + yrel : 0);
        y = (mouse->last_y + yrel < windowHeight ? y : windowHeight);
#else
        x = (mouse->last_x + xrel);
        y = (mouse->last_y + yrel);
#endif
